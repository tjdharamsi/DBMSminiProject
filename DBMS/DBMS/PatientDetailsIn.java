...
class PatientDetailsIn extends JFrame implements ActionListener
{
 ...
 PatientDetailsIn(String x)
 {
  ...
  bclr.addActionListener(new PatientDetailsIn.clear());
  bsub.addActionListener(new PatientDetailsIn.submit());
  bback.addActionListener(new PatientDetailsIn.back());
        ...
        public void actionPerformed(ActionEvent ae)
        {
         ...
         else
         {
            try
             ...
             new PatientDisplayIn(name,did1,name1,bloodgp,dob,area,city,did1,doa,gender,ph1,ph2,cur,rno);
             setVisible(false);
             rs2.close();
            stmt.close();
             c.close();
